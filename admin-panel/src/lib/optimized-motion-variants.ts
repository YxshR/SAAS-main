...
export function createOptimizedVariants(
  ...
  if (simplifyForLowEnd && !shouldUseGPU) {
    ...
    return Object.keys(baseVariants).reduce((acc, key) => {
      const variant = baseVariants[key] as any;
      acc[key] = {
  ...
  return Object.keys(baseVariants).reduce((acc, key) => {
    ...
    acc[key] = {
      ...variant,
      ...(useGPU && variant.transform && { transform: `${variant.transform} translate3d(0,0,0)` }),
      transition: customTransition || variant.transition || transitions.medium
    };
    return acc;
  }, {} as Variants);
}