...
export class NetworkErrorHandler {
  ...
  async handleRequest<T>(
        ...
        if (
          ...
          if (config.showToast && retryCount === 0) {
            toast.loading(`Connection issue detected. Retrying...`, {
              id: 'network-retry',
              description: `Attempt ${retryCount + 2} of ${retryConfig.maxRetries + 1}`,
            });
    ...
    if (lastError) {
      ...
      const fallbackError = new Error('Unknown network error') as NetworkError;
      fallbackError.name = 'NetworkError';
      this.handleFinalError(fallbackError, config);
      throw fallbackError;
    }
  }
