...
// Enhanced bounce variants for better visual feedback (Requirement 2.1, 2.2)
...
// Advanced bounce variants with different intensities
export const bounceIntensityVariants = {
  subtle: {
  ...
  strong: {
    hidden: { scale: 0.6, opacity: 0, rotate: -90 },
    visible: { 
      scale: 1, 
      opacity: 1,
      rotate: 0
...
// Enhanced hook for reduced motion with performance optimization (Requirement 8.3)
export const useReducedMotion = () => {
  ...
  useEffect(() => {
    ...
    // Use modern addEventListener with passive option for better performance
...
// Advanced reduced motion utilities (Requirement 8.3)
export const reducedMotionUtils = {
  // Create fallback animations for reduced motion
  createFallback: (normalVariants: Variants): Variants => ({
    hidden: { opacity: 0 },
    visible: { 
      opacity: 1, 
...
export const performanceMonitor = {
  ...
  getDeviceCapabilities: () => {
    ...
    return {
      tier: isHighEnd && !isMobile ? 'high' : isMobile ? 'low' : 'medium',
      renderer,
...
// Advanced animation utilities for complex interactions
export const advancedAnimationUtils = {
  // Create a sequence of animations with timing control
  createSequence: (animations: Array<{ variants: Variants; duration?: number; delay?: number }>) => {
    return animations.reduce((sequence, { variants, duration = durations.normal, delay = 0 }) => {
      return {
        ...sequence,
        transition: {
          ...sequence.transition,
  ...
  // Create morphing animations between shapes
  createMorphAnimation: (fromPath: string, toPath: string) => ({
    d: [fromPath, toPath],
    transition: {
  ...
  // Create particle-like animations
  createParticleAnimation: (count: number = 10) => {
...
// Advanced animation variants for complex interactions (Requirement 2.1, 2.2, 2.3)
